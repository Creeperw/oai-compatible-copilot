...
export function convertToolsToOpenAI(options?: vscode.ProvideLanguageModelChatResponseOptions): {
	...
	if (options?.toolMode === vscode.LanguageModelChatToolMode.Required) {
		if (tools.length !== 1) {
			console.error("[PolyLLM] ToolMode.Required but multiple tools:", tools.length);
...
export async function executeWithRetry<T>(fn: () => Promise<T>, retryConfig: RetryConfig): Promise<T> {
	...
	for (let attempt = 0; attempt <= maxAttempts; attempt++) {
		try {
			...
			console.error(
				`[PolyLLM] Retryable error detected, retrying in ${delayMs}ms (attempt ${attempt + 1}/${maxAttempts}). Error:`,
